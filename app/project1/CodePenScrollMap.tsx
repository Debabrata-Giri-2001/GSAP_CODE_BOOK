...
const CodePenScrollMap = () => {

    useGSAP(() => {

        // Set initial state for the circle (important for centering)
        gsap.set("#c", { xPercent: -50, yPercent: -50, transformOrigin: "50% 50%" });
        ...
        gsap
            .timeline({
                ...
                scrollTrigger: {
                    ...
                    scrub: 1,
                    onUpdate: self => {
                        // Get the current position of the circle on the path
                        const cX:any = gsap.getProperty('#c', 'x');
                        const cY:any = gsap.getProperty('#c', 'y');

                        // Calculate the center of the viewport
                        const centerX = window.innerWidth / 2;
                        const centerY = window.innerHeight / 2;

                        // Calculate the new position for the container
                        // This moves the container so the circle's current position
                        // aligns with the center of the viewport.
                        gsap.to('#container', {
                            x: centerX - cX,
                            y: centerY - cY,
                            duration: 0.7, // Maintain the quickTo duration
                            ease: 'none' // Or whatever ease you prefer for this follow
                        });
                    }
                }
            })
            .to('#c', {
                motionPath: {
                    path: '#p',
                    align: '#p', // Ensure the circle aligns *to* the path
                    alignOrigin: [0.5, 0.5] // Align the center of the circle to the path
                },
        ...
        // Remove the old quickTo and ticker.add as the onUpdate handles it now
        ...
        // center the container's left/top position initially, the onUpdate will adjust further
    ...
    return (
        <div className='w-[100%] h-[100%] overflow-x-hidden bg-[#f2efe9]'>
            <div id="scrollDist" className="absolute-element"></div>
            <div id="container" className="absolute-element">
                <img className="absolute-element" src="https://assets.codepen.io/721952/mtp.png" />
                <svg
...
export default CodePenScrollMap